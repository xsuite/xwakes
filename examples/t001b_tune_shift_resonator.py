...
p = xp.generate_matched_gaussian_bunch(line=line_no_wake,
            num_particles=10000, nemitt_x=1e-6, nemitt_y=1e-6, sigma_z=0.07,
...
line_no_wake.track(p0, num_turns=30000,
                   log=mylog,
                   with_progress=10)
log_no_wake = line_no_wake.log_last_track

line_with_wake.track(p, num_turns=30000,
                        log=mylog,
                        with_progress=10)
...
hist, z_bins = np.histogram(p.zeta, wake._wake_tracker.slicer.zeta_centers)
hist0, z_bins = np.histogram(p0.zeta, wake._wake_tracker.slicer.zeta_centers)
z_hist_cent = (z_bins[1:] + z_bins[:-1]) / 2

bunch_spectrum = np.fft.fft(hist)
frequencies = np.fft.fftfreq(len(z_hist_cent), d=(z_hist_cent[1] - z_hist_cent[0])/clight)
