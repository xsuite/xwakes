...
def get_component_name(is_impedance, plane, exponents):
    ...
    """
    component_names = wit.interface.component_names
...
def create_component_from_data(is_impedance: bool, plane: str, exponents: Tuple[int, int, int, int],
                               ...
                               extrapolate_to_zero: bool = False) -> wit.Component:
    """
    ...
    return wit.Component(impedance=(func if is_impedance else None),
...
def _iw2d_format_layer(layer: wit.Layer, n: int) -> str:
    """
...
def _iw2d_format_freq_params(params: wit.Sampling) -> str:
    """
...
def _iw2d_format_z_params(params: wit.Sampling) -> str:
    """
...
def create_iw2d_input_file(iw2d_input: Union[wit.FlatIW2DInput, wit.RoundIW2DInput], filename: Union[str, Path]) -> None:
    """
...
def check_already_computed(iw2d_input: Union[wit.FlatIW2DInput, wit.componentRoundIW2DInput],
...
def add_iw2d_input_to_database(iw2d_input: Union[wit.FlatIW2DInput, wit.RoundIW2DInput], input_hash: str,
...
def create_element_using_iw2d(iw2d_input: Union[wit.FlatIW2DInput, wit.RoundIW2DInput], name: str, beta_x: float, beta_y: float,
    ...
    return wit.Element(length=iw2d_input.length,
...
def _create_iw2d_input_from_dict(d: Dict[str, Any]) -> Union[wit.FlatIW2DInput, wit.RoundIW2DInput]:
...
def create_iw2d_input_from_yaml(name: str) -> Union[wit.FlatIW2DInput, wit.RoundIW2DInput]:
    """
...
def create_multiple_elements_using_iw2d(iw2d_inputs: List[wit.interface.IW2DInput], names: List[str],
                                        beta_xs: List[float], beta_ys: List[float]) -> List[wit.Element]:
    """
...
def create_htcondor_input_file(iw2d_input: wit.interface.IW2DInput, name: str, directory: Union[str, Path]) -> None:
    exec_string = ""
...
def load_longitudinal_impedance_datafile(path: Union[str, Path]) -> wit.Component:
    data = _read_cst_data(path)
    fs = data[:, 0]
    zs = data[:, 1] + 1j * data[:, 2]
    func = interp1d(x=fs, y=zs, kind='linear', assume_sorted=True, bounds_error=False, fill_value=(0., 0.))
    return wit.Component(impedance=func, plane='z', source_exponents=(0, 0), test_exponents=(0, 0))


def load_transverse_impedance_datafile(path: Union[str, Path]) -> Tuple[
                    wit.Component, wit.Component, wit.Component, wit.Component]:
    data = _read_cst_data(path)
    fs = data[:, 0]
    ...
    for i, z in enumerate(zs):
        ...
        components += (wit.Component(impedance=func,
...
def load_longitudinal_wake_datafile(path: Union[str, Path]) -> wit.Component:
    data = _read_cst_data(path)
    ts = data[:, 0]
    ws = data[:, 1] * 1e15
    func = interp1d(x=ts, y=ws, kind='linear', assume_sorted=True, bounds_error=False, fill_value=(0., 0.))
    return wit.Component(wake=func, plane='z', source_exponents=(0, 0), test_exponents=(0, 0))


def load_transverse_wake_datafile(path: Union[str, Path]) -> Tuple[
            wit.Component, wit.Component, wit.Component, wit.Component]:
    data = _read_cst_data(path)
    ts = data[:, 0]
    ...
    for i, w in enumerate(ws):
        ...
        components += (wit.Component(wake=func,
                                 plane='x' if i % 2 == 0 else 'y',
                                 source_exponents=(exponents[0], exponents[1]),
                                 test_exponents=(exponents[2], exponents[3])),)
