...
def test_sacherer_formula_function():
    ...
    def impedance_function(x):
        ...
        result[ind_p] = (numpy.interp(x[ind_p], impedance_data[:, 0], impedance_data[:, 1]) +
                         1j*numpy.interp(x[ind_p], impedance_data[:, 0], impedance_data[:, 2]))
        result[ind_n] = -((numpy.interp(np.abs(x[ind_n]), impedance_data[:, 0], impedance_data[:, 1]) +
                           1j*numpy.interp(np.abs(x[ind_n]), impedance_data[:, 0], impedance_data[:, 2]))).conjugate()
