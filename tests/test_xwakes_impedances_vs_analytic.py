...
from xwakes.wit.component import ComponentIndirectSpaceCharge
...
@pytest.mark.parametrize('wake_type', ['dipolar'])
@pytest.mark.parametrize('plane', ['x', 'y'])
def test_impedances_vs_analytic_indirect_space_charge_transverse(wake_type, plane):
    radius = 0.08
    gamma0 = 1.05
    length = 5
    kind = f'{wake_type}_{plane}'
    ...
    xo.assert_allclose(w_vs_zeta, w_vs_minus_zeta, rtol=0, atol=0)

    omega = np.linspace(-1e9, 1e9, 80)
    ...
    xo.assert_allclose(
        Z_from_zeta, Z_analytical, atol=0, rtol=1e-5)
    xo.assert_allclose(
        Z_from_t, Z_analytical, atol=0, rtol=1e-5)
