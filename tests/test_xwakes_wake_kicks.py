...
def test_wake_kick_indirect_space_charge(test_context, kind):
    # test that the indirect space charge wake kick is consistent with the
    # definition. In particular in this case the wake is non-zero in front of
    # the source particle
    b = 0.08
    gamma = 1.05
    L = 1
    ...
    # here the result is not really important so to speed up the test we use
    # a very large stopping criterion
    wf = xw.WakeIndirectSpaceCharge(kind = kind,
                                    length = L,
                                    gamma = gamma,
                                    radius = b,
                                    stop_crit = 1e-5)
    ...
    line = xt.Line(elements=[wf])
    line.build_tracker()
    line.particle_ref=xp.Particles(gamma0=gamma)

    # we use less particles than slices to speed up the test
    zeta_parts = wf.slicer.zeta_centers.flatten()[[0, num_slices//4,
                                                   num_slices//2,
                                                   3*num_slices//4,
                                                   -1]]

    particles = xp.Particles(gamma0=gamma,
                            zeta=zeta_parts,
                            weight=1e12,
                            _context=test_context)
...
def test_wake_in_front(test_context, kind):
    # we test the wake kick when the wake is in front of the source particle,
    # which is achieved by flipping the wake function of a resonator
    h_bunch = 600
    circumference=26658.883
    bucket_length = circumference/h_bunch/10
    zeta_range = (-0.5*bucket_length, 0.5*bucket_length)
    ...
    for icomp, component in enumerate(wf.components):
        comp_pos = wf_pos.components[icomp]
        component.function_vs_zeta = (
            lambda zeta, dzeta, beta0: comp_pos.function_vs_zeta(zeta=-zeta,
                                                                 beta0=beta0,
                                                                 dzeta=dzeta))
    ...
    zeta_parts = wf.slicer.zeta_centers.flatten()
    ...
    # we displace before a certain particle index so that we check also that
    # in the dipolar and quadrupolar cases there is no wake in behind (opposite of
    # wake in front)
    particles.x[:i_source_x] += displace_x
    particles.y[:i_source_y] += displace_y
